...
console.log(allNumebers);
 // lastTime = document.getElementById("closeTime");
...
//chart
	x_scale= d3.scaleTime()
      ...
      // x_scale.ticks(d3.timeMinute.every(5));



    // visualizeTicks(x_scale, [10, d3.timeFormat("%S")])
    ...
    //high area
        ...
        .attr("id", "high_area")
    		.attr("d", d3.area()
    			...
    			.y0(function(d){return y_scale(d.low)})
    			.y1(function(d){ return y_scale(d.high)}))
    		.attr("transform", "translate("+margin.left*2+","+(-margin.bottom*2)+")");
...
d3.select("#high_area")
  ...
  .attr("d", d3.area()
    ...
    .y0(function(d){return new_y_scale(d.low)})
    .y1(function(d){ return new_y_scale(d.high)}));
...
function drawRect()
{
	...
	.on("mousemove", (event)=>{
		...
		index= bisecting(timeCloseArr, selectedx );
